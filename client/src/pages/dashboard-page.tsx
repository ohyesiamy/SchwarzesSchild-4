          ...
          <div className="lg:col-span-8 space-y-5 lg:border-r border-gray-200 lg:pr-6">
            ...
            <section className="bg-white border border-gray-200 shadow-sm mb-5">
              ...
              <div className="hidden md:block">
                  ...
                  <tbody>
                    {recentTransactions.map((transaction, index) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <div className={`w-6 h-6 flex items-center justify-center rounded-full mr-2 ${
                              ...
                              <span className="text-[10px] font-medium">{transaction.amount < 0 ? '-' : '+'}</span>
                            </div>
                            <div>
                        ...
                        <td className="px-4 py-3 text-xs">{formatDate(transaction.date)}</td>
                        <td className="px-4 py-3">
                          <span className="text-[10px] uppercase tracking-wide px-2 py-0.5 bg-gray-100 rounded-sm">{transaction.category}</span>
                        </td>
                        <td className={`px-4 py-3 text-right font-medium ${transaction.amount < 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {formatCurrency(transaction.amount, transaction.currency)}
                        </td>
                        <td className="px-4 py-3">
                ...
                <div className="flex justify-between items-center px-4 py-2 border-t border-gray-200 bg-gray-50 text-[10px] uppercase tracking-wide text-gray-500">
                  <span>Showing {recentTransactions.length} most recent transactions</span>
                  <Button 
                    variant="link" 
                    size="sm" 
                    className="text-[10px] uppercase tracking-wide p-0 h-auto"
                    onClick={() => navigate("/transactions")}
                  >
                    View all
                  </Button>
            ...
            <section className="bg-white border border-gray-200">
              <div className="flex flex-col xs:flex-row justify-between items-start xs:items-center p-2.5 border-b border-gray-200">
                <div>
                  <h2 className="text-xs uppercase tracking-wide font-medium">Banking Services</h2>
                  <p className="text-[10px] text-gray-500 hidden md:block mt-0.5">Quick access to essential financial services</p>
                </div>
              </div>
              
              <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                <div 
                  className="bg-white border border-gray-200 hover:border-black text-center p-3 md:p-4 rounded-none transition-colors duration-200 cursor-pointer active:bg-gray-50"
                  onClick={() => navigate("/transactions")}
                >
                  <div className="flex flex-col items-center justify-center">
                    <div className="w-8 h-8 rounded-full bg-black mb-2 flex items-center justify-center">
                      <PlusIcon className="h-4 w-4 text-white" />
                    </div>
                    <span className="font-medium text-xs uppercase tracking-wide mb-1">Transfers</span>
                    <span className="text-[10px] text-gray-600">Send payments</span>
                  </div>
                </div>
                ...
                >
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="font-medium text-xs uppercase tracking-wide mb-1">Deposits</span>
                    <span className="text-[10px] text-gray-600">Fund your accounts</span>
                  </div>
                </div>
